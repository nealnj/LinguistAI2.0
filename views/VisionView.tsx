...
const VisionView: React.FC = () => {
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center p-20 animate-in fade-in">
        <div className="relative mb-8">
           ...
           <Globe className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-indigo-400" size={48} />
        </div>
        <div className="text-center space-y-3">
          <h3 className="text-3xl font-black text-slate-800 tracking-tighter">正在同步全球脉动...</h3>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">AI 正在扫描 2024-2025 全球资讯、音乐与影视动态</p>
        </div>
      </div>
    );
  }
  ...
  // 渲染深度解析视图
  if (selectedItem && (analyzing || analysis)) {
    return (
      <div className="max-w-7xl mx-auto space-y-10 pb-20 animate-in slide-in-from-right duration-500">
        <button onClick={closeAnalysis} className="flex items-center gap-3 text-slate-400 hover:text-slate-800 font-black text-xs uppercase tracking-[0.2em] transition-all">
          <ArrowLeft size={16} /> 返回趋势发现
        </button>
        ...
        {analyzing ? (
          <div className="bg-white rounded-[4rem] p-32 border border-slate-100 flex flex-col items-center justify-center gap-8 shadow-sm">
             ...
             <div className="text-center space-y-2">
               ...
               <p className="text-slate-400 text-sm font-bold uppercase tracking-widest">Generating Vocabulary, Collocations, Expressions & Grammar Analysis</p>
             </div>
          </div>
        ) : analysis && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-5 space-y-10">
              ...
              <div className="bg-slate-900 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden group">
                 ...
                 <h4 className="text-xl font-black mb-6 relative z-10">AI 学习指引</h4>
                 <p className="text-sm text-slate-400 leading-relaxed italic relative z-10">
                   “本文选自当前最热的 {selectedItem.type === 'news' ? '资讯' : selectedItem.type === 'song' ? '流行乐' : '电影'} 话题。建议重点掌握『高级句式』中的逻辑引导词，并在『核心词汇』中挑选 2 个单词用于今日的口语练习。”
                 </p>
              </div>
            </div>
          </div>
    ...
    <div className="max-w-7xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-12">
          ...
          <div className="flex flex-col bg-white/5 p-8 rounded-[3rem] border border-white/10 backdrop-blur-xl space-y-6 min-w-[320px]">
             <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-slate-400">
               <span>当前扫描状态</span>
               <span className="text-emerald-400 flex items-center gap-1"><CheckCircle2 size={12}/> 实时同步中</span>
             </div>
      ...
      {error ? (
        <div className="bg-white rounded-[4rem] p-24 border border-slate-100 text-center space-y-6 shadow-sm">
           ...
           <button onClick={fetchData} className="px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black flex items-center gap-2 mx-auto"><RefreshCcw size={20}/> 重试扫描</button>
        </div>
      ) : trends && (
          ...
          {activeTab === 'news' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {trends.news?.map((n: any, i: number) => (
                <div key={i} className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm hover:shadow-2xl transition-all group relative overflow-hidden flex flex-col h-full">
          ...
          {activeTab === 'songs' && (
             <div className="space-y-8">
               {trends.songs?.map((s: any, i: number) => (
                 <div key={i} className="bg-white rounded-[4rem] p-12 border border-slate-100 shadow-sm grid grid-cols-1 lg:grid-cols-12 gap-12 group">
          ...
          {activeTab === 'movies' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              {trends.movies?.map((m: any, i: number) => (
                <div key={i} className="bg-white rounded-[4rem] p-12 border border-slate-100 shadow-sm hover:shadow-2xl transition-all space-y-8 flex flex-col">
