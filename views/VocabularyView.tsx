...
const VocabularyView: React.FC<{ onNavigate?: (module: LearningModule) => void }> = ({ onNavigate }) => {
  ...
  if (reviewMode && currentWord) return (
    <div className="max-w-xl mx-auto py-12 px-4 animate-slide-up">
      <div className="bg-white rounded-[3.5rem] shadow-2xl p-12 border border-slate-100 text-center space-y-8">
        <h2 className="text-5xl font-black text-slate-800 tracking-tighter">{currentWord.translation}</h2>
        <input autoFocus value={userInput} onChange={(e) => setUserInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && checkReview()} placeholder="输入英文单词..." className={`w-full bg-slate-50 border-4 rounded-2xl p-6 text-center text-4xl font-black outline-none transition-all ${reviewFeedback === 'correct' ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : reviewFeedback === 'wrong' ? 'border-rose-500 bg-rose-50 text-rose-700' : 'border-slate-100 focus:border-indigo-600'}`} />
  ...
  return (
    <div className="max-w-7xl mx-auto flex flex-col gap-8 animate-in fade-in duration-700 pb-20">
      <div className="bg-white px-10 py-6 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between">
        <div className="flex items-center gap-6">
          <div className="bg-indigo-600 p-4 rounded-xl text-white shadow-lg"><BookOpen size={20} /></div>
          <div>
            <h1 className="text-2xl font-black text-slate-800">词汇实验室</h1>
            <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">Progress: {currentIndex + 1}/{words.length}</p>
          </div>
        </div>
      ...
      {currentWord && (
        <div className="grid grid-cols-12 gap-8 items-start">
          <div className="col-span-12 lg:col-span-7 space-y-8">
            <div className="bg-white rounded-[4rem] shadow-xl p-16 border border-slate-100 text-center space-y-6">
              <span className="px-3 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-black rounded-full uppercase">{currentWord.pos}</span>
              <h2 className="text-8xl font-black text-slate-800 tracking-tighter">{currentWord.word}</h2>
              <p className="text-4xl font-black text-indigo-600">{currentWord.translation}</p>
              ...
              {currentWord.imageUrl && <div className="rounded-[3rem] overflow-hidden shadow-inner border border-slate-50 min-h-[400px]"><img src={currentWord.imageUrl} className="w-full h-auto object-cover" alt="visual mnemonic" /></div>}

              <div className="bg-slate-50 p-10 rounded-[3rem] text-left space-y-4">
                <h4 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">学术语境</h4>
                <p className="text-2xl italic text-slate-800 font-black">"{currentWord.example}"</p>
                <p className="text-slate-500 font-bold">{currentWord.exampleTranslation}</p>
              </div>

              <div className="flex gap-4">
                <button disabled={currentIndex === 0} onClick={() => setCurrentIndex(currentIndex - 1)} className="p-7 rounded-[2rem] border border-slate-100 text-slate-300 disabled:opacity-20"><ChevronLeft size={24} /></button>
                <button onClick={handleNext} className="flex-1 py-7 rounded-[2rem] bg-slate-900 text-white font-black text-xl shadow-2xl flex items-center justify-center gap-3">{currentIndex === words.length - 1 ? '复现练习' : '下一词'} <ChevronRight size={24} /></button>
              </div>
            </div>
          </div>

          <div className="col-span-12 lg:col-span-5 space-y-8">
            <div className="bg-white rounded-[3rem] shadow-lg overflow-hidden flex flex-col border border-slate-100">
              <div className="flex bg-slate-50/50 p-2 gap-2">
                {['systematic', 'memory', 'info'].map(tab => (
                  <button key={tab} onClick={() => setActiveTab(tab as any)} className={`flex-1 py-4 rounded-xl text-[10px] font-black uppercase transition-all ${activeTab === tab ? 'bg-white text-indigo-600 shadow-md border border-indigo-50' : 'text-slate-400'}`}>
                    {tab === 'systematic' ? '形态' : tab === 'memory' ? '关联' : '词根'}
                  </button>
                ))}
              </div>
              <div className="p-10 space-y-6 max-h-[600px] overflow-y-auto scrollbar-hide">
                {activeTab === 'systematic' && currentWord.forms?.map((f, i) => (
                  <div key={i} className="bg-indigo-50/30 p-6 rounded-2xl border border-indigo-100/50 space-y-3">
                    <div className="flex justify-between font-black">
                      <span className="text-xl text-indigo-700">{f.form}</span>
                      <span className="text-[10px] text-indigo-400 uppercase bg-white px-2 py-1 rounded-full">{f.pos}</span>
                    </div>
                    <p className="text-sm text-slate-800 font-bold">{f.meaning}</p>
